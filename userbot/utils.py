...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        mod.delete_eviral = delete_eviral
        mod.eod = delete_eviral
        mod.admin_cmd = admin_cmd
        mod.eviral_cmd = admin_cmd
        mod.sudo_cmd = sudo_cmd
        # support for FIREX originals
        sys.modules["FIREX.utils"] = userbot.utils
        sys.modules["FIREX"] = userbot
...
async def delete_eviral(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        eviralevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await eviralevent.delete()
