...
async def promote(promt):
    ...
    eviralevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await eviralevent.edit(NO_PERM)
        return
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    eviralevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
...
async def ban(bon):
    ...
    if not user:
        return
    eviralevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await eviralevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await eviralevent.edit(f"{str(user.id)} ιѕ ϐαииє∂ !!\nяєαѕοи: {reason}")
    else:
        await eviralevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    eviralevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await eviralevent.edit("υиϐαииє∂ ѕυϲϲєѕѕƒυℓℓγ! Giving one more chance 😏")
        if BOTLOG:
        ...
        await eviralevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    eviralevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await eviralevent.edit(f"{NO_PERM}\n{str(e)}")
        return
    if reason:
        await eviralevent.edit(
        ...
        await eviralevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
