...
from FIREX.utils import admin_cmd, progress
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit(f"Local file compressed to `{directory_name}.rar`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit(f"Local file compressed to `{output}`")
