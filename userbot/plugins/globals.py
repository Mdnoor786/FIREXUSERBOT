...
gbpic = Config.ALIVE_PIC or eviral_logo3
...
async def _(eviralevent):
    i = 0
    await eviralevent.get_sender()
    me = await eviralevent.client.get_me()
    eviral = await eor(eviralevent, "`Promoting globally...`")
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    f"@{me.username}" if me.username else my_mention
    await eviralevent.get_chat()
    if eviralevent.is_private:
        user = eviralevent.chat
        rank = eviralevent.pattern_match.group(1)
    else:
        eviralevent.chat.title
    try:
        user, rank = await get_full_user(eviralevent)
    except:
        pass
    if me == user:
        await eviral.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "ℓεɠεɳ∂"
    except:
        return await eviral.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await eviralevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await eviralevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await eviral.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await eviral.edit("**Reply to a user !!**")
    await eviral.edit(
...
async def _(eviralevent):
    ...
    await eviralevent.get_sender()
    me = await eviralevent.client.get_me()
    eviral = await eor(eviralevent, "`Demoting Globally...`")
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    f"@{me.username}" if me.username else my_mention
    await eviralevent.get_chat()
    if eviralevent.is_private:
    ...
    try:
        ...
        pass
    if me == user:
        await eviral.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "eviral"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await eviralevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await eviral.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await eviral.edit("**Reply to a user !!**")
...
async def _(event):
    eviral = await eor(event, "`Gbanning...`")
    reason = ""
    ...
    if event.reply_to_msg_id:
        ...
        return await eod(
            eviral, "**To gban a user i need a userid or reply to his/her message!!**"
        )
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == Eviral:
        return await eod(eviral, "🥴 **Nashe me hai kya lawde ‽**")
    if str(userid) in DEVLIST:
        return await eod(eviral, "😑 **GBan my creator ?¿ Really‽**")
    ...
    if a is not None:
        ...
        gbpic = eviral_logo2
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **Gbanned** By {eviral_mention} **Reason = Not Mentioned 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {eviral_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(eviral, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eor(eviral, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                await event.client.edit_permissions(
...
async def already(event):
    ...
    if len(gbanned_users) > 0:
        for user in gbanned_users:
            ...
            eviral = int(hel)
            try:
                tity = await event.client.get_entity(eviral)
                name = tity.first_name
            except ValueError:
                name = "User"
            GBANNED_LIST += f"📍 [{name}](tg://user?id={eviral}) (`{eviral}`)\n"
    else:
...
async def gkick(event):
    eviral = await eor(event, "`Kicking globally...`")
    ...
    if event.reply_to_msg_id:
        ...
        return await eor(eviral, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == Eviral:
        return await eod(eviral, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eor(eviral, "**😪 I'm not going to gkick my developer!!**")
