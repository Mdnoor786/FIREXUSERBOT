...
async def fs(event):
    ...
    if not await is_admin(event, eviralbooy):
    ...
    if args in ("off", "no", "disable"):
        ...
        rip = await check_him(channel, eviralbooy)
        if rip is False:
            return await event.reply(
    ...
    if chat_db:
        try:
            ...
            if rip is False:
                ...
                eviralboy = Config.BOT_USERNAME
                await bot(
                    EditBannedRequest(event.chat_id, event.sender_id, MUTE_RIGHTS)
                )
                response = await bot.inline_query(eviralboy, "pm_warn")
...
@bot.on(events.InlineQuery(pattern="eviralbooy"))
async def NOOBBOY(event):
    global rk
    event.builder
    eviralproboy = [[Button.inline("Unmute Me 😊", data="eviral")]]
    NOOBBOYOP = eviralboy.article(
        title="FORCE SUBSCRIBE", text="fsub", buttons=eviralproboy
    )
...
@bot.on(events.callbackquery.CallbackQuery(re.compile(b"eviral")))
